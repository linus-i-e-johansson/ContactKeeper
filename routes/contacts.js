...
router.get("/", auth, async (req, res) => {
  try {
    ...
    const contact = await Contact.find({ user: req.user.id }).sort({
      date: -1,
    }); // sorted by date, most recent one first.
    res.json(contact);
  } catch (e) {
...
router.put("/:id", auth, async (req, res) => {
  ...
  try {
    ...
    if (!contact) {
      // if there is no contact by that id.
...
router.delete("/:id", auth, async (req, res) => {
  ...
  try {
    ...
    //Makes sure the user owns the contact being modified.
    if (contact.user.toString() !== req.user.id) {
      return res.status(401).json({ msg: "Not authorized" }); //if they dont match.
    }
    //perform the actuall deletion of the contact.
